...
def main():
    ...
    while minimum < 0:
        ...
        y = np.linalg.solve(BT, N[entering_var_index])
        if (y <= 0).all():
            print("ilimitada")

        print(y)
        # ratio_test = []
        ratio_test = float('inf')
        for i in range(len(basic_variables)):
            # ratio_test.append(b[i] / y[i])
            if (b[i] / y[i]) < ratio_test:
                ratio_test = (b[i] / y[i])
                ratio_test_index = i

        print(ratio_test)
        # print(ratio_test_index)
        # print(ratio_test.index(min(ratio_test)))


        # O QUE FAZER????
        leaving_var_index = ratio_test_index # n_rows + ratio_test.index(min(ratio_test))
